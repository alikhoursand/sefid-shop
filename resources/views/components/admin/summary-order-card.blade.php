...
<div class=" bg-base-100 shadow-md shadow-base-300 p-4 rounded-box">
    <div class="flex gap-x-2 items-center justify-between">
        ...
        <div class="divider my-2 grow"></div>
        <span class="font-medium">{{ $order->id }}</span>
    </div>
    <div class="flex gap-x-2 items-center justify-between">
        <span class="opacity-75  ">تاریخ سفارش</span>
        <div class="divider my-2 grow"></div>
        <span class="font-medium">{{ verta($order->updated_at)->format('%d %B %Y') }}</span>
    </div>
    <div class="flex gap-x-2 items-center justify-between">
        ...
        <div>
            <span
                class="font-medium">{{ number_format($order->cost + $order->tax + $order->post - $order->discount) }}</span>
