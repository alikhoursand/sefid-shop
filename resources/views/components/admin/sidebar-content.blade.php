<div class="h-full px-2 py-4 overflow-y-auto bg-base-100 w-64 space-y-1">
    ...
    @if (auth()->user()->hasAccess('superadmin'))
        <div class="collapse collapse-arrow">
            <input type="checkbox" name="menu-drop_{{ $type }}" @checked(in_array(Route::currentRouteName(), ['admin.user.list', 'admin.user.admin.list'])) />
            <div class=" collapse-title btn btn-block justify-start btn-ghost">
                <x-heroicon-s-user-group class="size-6 @if (in_array(Route::currentRouteName(), ['admin.user.list', 'admin.user.admin.list'])) text-primary @endif" />
    ...
    <div>
        <a href="{{ route('admin.faq.list') }}"
            class="{{ Route::currentRouteName() === 'admin.faq.list' ? 'btn-primary' : 'btn-ghost' }} btn btn-block justify-start">
            <x-heroicon-s-question-mark-circle class="size-6" />
