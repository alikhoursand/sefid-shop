    ...
    <div class="collapse-title  flex flex-col gap-y-4 md:flex-row justify-between md:items-center items-start">
        ...
        <div class="text-center basis-1/2 sm:basis-1/3 lg:basis-3/12">
            @if ($order->status == 1 || $order->status == 2)
                <div class="badge lg:badge-sm font-medium xl:badge-md badge-info">
                    <span>در انتظار پرداخت</span>
                </div>
            @elseif($order->status == 3)
                <div class="badge lg:badge-sm font-medium xl:badge-md badge-success">
                    <span>پرداخت شده</span>
                </div>
            @elseif($order->status == 4)
                <div class="badge lg:badge-sm font-medium xl:badge-md badge-error">
                    <span>لغو شده</span>
                </div>
            @elseif($order->status == 5)
                <div class="badge lg:badge-sm font-medium xl:badge-md badge-primary">
                    <span>تکمیل شده</span>
                </div>
            @endif
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-y-2 flex-wrap pt-2 sm:pt-4 text-sm md:text-base">
            ...
            <div class="basis-full p-2 ">
                <div class="font-medium flex flex-col gap-y-2">
                    ...
                    <div>
                        <span class="opacity-75 font-normal">کد پستی:</span>
                        {{ $address->postal_code }}
                    </div>
                    <div>
                        <span class="opacity-75 font-normal">شماره تماس:</span>
                        {{ $order->user->phone }}
                    </div>
                    <div><span class="opacity-75 font-normal">شهر - استان:</span>
                        {{ getLocation($address->city_id) . ' - ' . getLocation($address->state_id) }}
                    </div>
                    <div><span class="opacity-75 font-normal">آدرس:</span> {{ $address->address }}
                    </div>
                </div>
            </div>
