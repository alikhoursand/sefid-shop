...
@section('content')
    <x-admin.page-title :page_title="'ویرایش محصول'" :return="'admin.shop.product.index'" :breadcrumbs="[
        [
            'title' => 'محصولات',
        ...
        <form action="{{ route('admin.shop.product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex flex-row flex-wrap">
                <div class="xl:basis-1/4 lg:basis-1/3 md:basis-1/2 w-full p-2">
                    ...
                    <select id="category_id" name="category_id" class="select focus:outline-none w-full">
            ...
            <div class="flex-row flex-wrap flex">
                <div class="xl:basis-1/4 lg:basis-1/3 md:basis-1/2 basis-full p-2">
                    <label for="price" class="block mb-2">قیمت</label>
                ...
                <div class="xl:basis-1/4 lg:basis-1/3 md:basis-1/2 w-full p-2">
                    ...
                    <input type="number" id="off_price" name="off_price" placeholder="قیمت با تخفیف"
                        value="{{ $product->off_price }}" class="input focus:outline-none w-full">
                    <div class="error text-xs md:text-sm text-red-500 mt-1">
                        @error('off_price')
                            {{ $message }}
                        @enderror
            ...
            <div class="p-2 mt-5 lg:w-2/3 w-full">
                ...
                <textarea class="textarea focus:outline-none w-full" name="desc" id="desc" placeholder="توضیحات محصول">{{ $product->desc }}</textarea>
                <div class="error text-xs md:text-sm text-red-500 mt-1">
                    @error('desc')
                        {{ $message }}
                    @enderror
            ...
            <div class="p-2 mt-5 xl:w-1/3 lg:w-1/2 md:w-2/3 w-full">
                ...
                <label for="image" class="block mb-2">عکس
                    شاخص</label>
                <input name="image" class="file-input w-full max-w-[350px] block mt-2 focus:outline-none" id="image"
                    type="file">
                <div class="error text-xs md:text-sm text-red-500 mt-1">
                    @error('image')
                        {{ $message }}
                ...
                <div class="mt-5">
                    <img id="imagePreview" class="" src="{{ Storage::url($product->image) }}" alt="پیش نمایش">
                </div>
            </div>

            <button class="btn btn-success mt-10">ویرایش محصول</button>
        </form>
    </div>
@endsection
